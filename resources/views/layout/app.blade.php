        ...
        <ul class="navbar-nav navbar-nav-right">
          ...
          <li class="nav-item nav-profile dropdown">
            <a class="nav-link dropdown-toggle" href="#" data-toggle="dropdown" id="profileDropdown">
              <img src="{{ asset('profile_pic/'.Auth::user()->img) }}" alt="profile"/>
...
@foreach(App\Models\User::get() as $user)
              <li class="list active">
                <div class="profile"><img src="{{ asset('profile_pic/'.$user->img) }}" alt="image"><span class="{{ $user->isActive() ? 'online' : 'offline' }}"></span></div>
                <div class="info">
                  <p><a href="{{ url('/chatroom/'.$user->id) }}">{{ $user->name }}</p></a>
...
@endforeach
          ...
          <div class="tab-pane fade " id="todo-section" role="tabpanel" aria-labelledby="todo-section">
            <div class="add-items d-flex px-3 mb-0">
              <form action="{{ route('addgroup') }}" method="post" class="form w-100">
                @csrf
...
@foreach ($ids as $id)
          ...
          @endphp
              ...
              <li class="list active">
                ...
                <div class="info">
                  <a href="{{ url('/groupchatroom/'.$group->id) }}">
                  <h2>{{ $group->name }}</h2>
                  </a>
                  @if($group->onwer_id == Auth::id())
                  <p>Admin</p>
                  <a href="{{ url('/groupdelet/'.$group->id) }}">Delete</a>
                  @else
                  <p>Member</p>
                  @endif
                </div>
